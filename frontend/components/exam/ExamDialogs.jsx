// src/components/exam/ExamDialogs.jsx
import { motion } from "framer-motion";
import { AlertTriangle, Loader2, Send, Lock as LockIcon } from "lucide-react";

/* ─────────────────────────────────────────
   Full-exam submit confirmation
───────────────────────────────────────── */
export function SubmitDialog({ answered, total, onConfirm, onCancel, submitting }) {
      ...
      >
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onConfirm}
            disabled={submitting}
            className="flex-1 py-2.5 rounded-xl bg-indigo-500 hover:bg-indigo-400 text-sm font-semibold
              text-white disabled:opacity-60 transition-colors flex items-center justify-center gap-2"
          >
            {submitting
...
/* ─────────────────────────────────────────
   Strict-section submit confirmation
───────────────────────────────────────── */
export function SectionSubmitDialog({
  sectionName,
  answeredInSection,
  totalInSection,
  isLastSection,
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-0.5">
              You{" "}
              <span className="text-amber-400 font-semibold">cannot return</span>
              {" "}to this section after continuing.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onConfirm}
            className="flex-1 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-sm font-semibold
              text-zinc-950 transition-colors flex items-center justify-center gap-2"
          >
            <LockIcon size={13} />
