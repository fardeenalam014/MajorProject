...
export default function StudentDashboard() {
  ...
  const unenrollTest = async (enrollmentId, testTitle) => {
    ...
    // If your backend has DELETE /enrollments/:id, call it here.
    // For now we just remove from local state since the enrollment
    // stays on the server (student can re-join with same code).
    // Uncomment the line below if you add that route:
      ...
      <main className="flex-1 flex flex-col overflow-y-auto"
        ...
        <div className="sticky top-0 z-10 h-14 flex items-center justify-between px-6
          ...
          <div>
            ...
            <p className="mono text-[10px] text-zinc-600 mt-0.5">{authUser?.email}</p>
          </div>
          <button onClick={logout}
