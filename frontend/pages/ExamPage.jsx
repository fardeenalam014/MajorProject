 ...
 * Strict section support: locks back nav, confirm dialog, auto-advance locks
 */
...
function ResultScreen({ result, onDone }) {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif" }} className="h-screen bg-zinc-950 flex items-center justify-center p-4">
      <FontLoader />
...
export default function ExamPage() {
  ...
  const advanceSection = useCallback((fromIdx, lockIt = false) => {
    // Always lock if lockIt is true (strict section submitted or timed out)
    ...
    // Find next navigable section: prefer next index first, then scan all
    // A section is navigable if it's not locked and not strict+timed
    const findNextNavigable = (locked) => {
      // First try sections after fromIdx in order
      for (let i = fromIdx + 1; i < sections.length; i++) {
        const s = sections[i];
        if (!locked.has(i) && !(s.strictTimer && s.duration > 0)) return i;
      }
      // Then try sections before fromIdx (non-strict, not locked)
      for (let i = 0; i < fromIdx; i++) {
        const s = sections[i];
        if (!locked.has(i) && !(s.strictTimer && s.duration > 0)) return i;
      }
      // Then any unlocked section (including strict ones that haven't been locked)
      for (let i = 0; i < sections.length; i++) {
        if (i === fromIdx) continue;
        if (!locked.has(i)) return i;
      }
      return null; // all sections locked
    };
    ...
    if (nextIdx !== null) {
      ...
      // No navigable sections left — auto submit
      setShowSubmit(true);
    }
  ...
  const startScreenFeed = useCallback(async () => {
    try {
      ...
      stream.getTracks().forEach(t => { pc.addTrack(t, stream); t.onended = () => { screenPCRef.current?.close(); setScreenSharing(false); }; });
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
  ...
  const connectSocket = useCallback(() => {
    const token  = localStorage.getItem("token_student") ?? localStorage.getItem("token_creator") ?? localStorage.getItem("token");
  ...
  useEffect(() => {
    const h = () => {
      ...
      navigator.sendBeacon(`${SOCKET_URL}/api/attempts`,
        new Blob([JSON.stringify({ testId, timeTaken, abandoned: true, answers: answersRef.current,
          violations: { tabSwitches: tabRef.current, faceWarnings: faceRef.current, deviceFlags: devRef.current, fullscreenExits: escRef.current } })],
  ...
  useEffect(() => () => {
    cameraStreamRef.current?.getTracks().forEach(t => t.stop());
    screenStreamRef.current?.getTracks().forEach(t => t.stop());
    cameraPCRef.current?.close(); screenPCRef.current?.close();
    socketRef.current?.disconnect();
    clearInterval(totalTimerRef.current); clearInterval(sectionTimerRef.current);
  }, []);
  ...
  // Nearest non-locked section before current (skip locked ones)
  ...
  // Can go back only if current section is non-strict and there's a non-locked previous section
  const canGoBack = prevNonLockedIdx !== null && !isStrictSection;

  // Nearest non-locked section after current (for forward arrow)
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif" }}
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside className="w-56 shrink-0 flex-col border-r border-zinc-800 bg-zinc-950 hidden md:flex overflow-y-auto"
            ...
            {!started ? (
              ...
              <>
                ...
                <div className="absolute bottom-2 left-2 flex gap-1">
                  <span className="flex items-center gap-0.5 bg-black/70 px-1.5 py-0.5 rounded mono text-[9px] text-emerald-400"><Camera size={8} /> ON</span>
                  {screenSharing && <span className="flex items-center gap-0.5 bg-black/70 px-1.5 py-0.5 rounded mono text-[9px] text-indigo-400"><Monitor size={8} /> SCR</span>}
                </div>
              </>
            )}
          </div>
          ...
          <div className="flex-1 p-3 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              {["Stay in fullscreen","One person in camera","No phones or books","Don't switch tabs","Screen is being shared"].map(r => (
        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          {/* section tabs */}
          {sections.length > 1 && (
            <div className="shrink-0 flex items-center gap-2 px-4 py-3 border-b border-zinc-800 overflow-x-auto bg-zinc-950" style={{ scrollbarWidth: "none" }}>
              {sections.map((sec, si) => {
                ...
                // A tab is clickable only if: not locked AND (current section is non-strict OR target is non-strict)
                const isClickable  = !isLocked && !(isStrictSection && isStrictSec && si !== activeSectionIdx);
                return (
                  <button key={sec._id}
                    disabled={!isClickable}
                    onClick={() => { if (isClickable) setActiveSectionIdx(si); }}
                    className={`shrink-0 flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-medium border transition-all
                      ${isActive
                    ...
                    {isLocked && <LockIcon size={9} className="shrink-0" />}
                    {sec.title}
                    <span className={`mono text-[9px] ${isActive ? "text-indigo-400" : "text-zinc-700"}`}>
          ...
          <aside className="w-16 shrink-0 border-l border-zinc-800 bg-zinc-950 flex-col hidden sm:flex overflow-y-auto"
            ...
            <div className="flex-1 p-2 flex flex-col gap-1.5">
              {sections.map((sec, si) => {
                const isLocked    = lockedSections.has(si);
                const isStrictSec = sec.strictTimer && sec.duration > 0;
                // Clickable from palette if: not locked AND (not currently in strict section OR target is same section OR target is non-strict)
                const canClickFromPalette = !isLocked && !(isStrictSection && isStrictSec && si !== activeSectionIdx);
                return (
                  <div key={sec._id}>
                    {sections.length > 1 && (
                      <p className="mono text-[8px] uppercase tracking-widest text-center mb-1 truncate px-1"
                        style={{ color: isLocked ? "#52525b" : "#3f3f46" }}>
                        {sec.title.slice(0, 3)}{isLocked ? " 🔒" : ""}
                      </p>
                    )}
                    {sec.questions.map((_, qi) => {
                      ...
                      return (
                        <button key={key}
                          disabled={!canClickFromPalette}
                          onClick={() => {
                            if (!canClickFromPalette) return;
